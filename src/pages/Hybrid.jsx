...
export default function Hybrid() {
  ...
  const fetchHybridJobs = async () => {
    try {
      ...
      const hybridJobs = all.filter(
        (job) => job.jobType?.trim().toLowerCase() === "hybrid"
      );

      setJobs(hybridJobs);
    } catch (err) {
      console.error("Hybrid jobs error:", err);
      toast.error("Failed to load hybrid jobs");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section className="pt-28 pb-12 max-w-7xl mx-auto px-4">
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6 shadow-sm">
        <h1 className="text-3xl font-bold text-emerald-800">Hybrid Jobs</h1>
        <p className="text-slate-600 mt-2 max-w-2xl">
          Enjoy freedom + collaboration. Hybrid roles mix remote work with
          in-office teamwork.
        </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-500">No hybrid jobs available.</div>
      ) : (
